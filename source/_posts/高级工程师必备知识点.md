...
* 接⼝⾥可以有静态⽅法和⽅法体
* 接⼝中所有的⽅法必须是抽象⽅法（`JDK8`之后就不是，可以`public abstract`抽象方法，静态方法，静态代码块，`default`方法）
* 接⼝不是被类继承了，⽽是要被类实现
* 接⼝⽀持多继承, 类不⽀持多个类继承
...
|集合|数据结构|是否安全|优缺点|使用场景|
|---|---|---|---|---
|`ArrayList`|数组|否|查询和修改非常快，但是增加和删除慢，数组元素分配在内存中的内存地址是连续的，根据索引脚标查询数据，但是新增和删除则会移动每个元素的内存地址位置，效率比较慢|查询和修改多则用`ArrayList`|
|`LinkedList`|双向链表|否|查询和修改非常快，但是增加和删除慢|查询和修改多则用`ArrayList`|
|`Vector`|数组|是，操作的时候使用`synchronized`进行加锁||多线程场景|
...
![MQ消息模式-发布订阅](https://volc1612.gitee.io/blog/images/高级工程师必备知识点/MQ消息模式-发布订阅.png)
...
参考： https://juejin.im/post/5b8f05a5f265da43296c6102
